...
using Devinno.Communications.TextComm;
using System;
...
{
    public class TextCommRTUMaster2 : MasterScheduler
    {
        #region class : WorkTC
        ...
        public class ReceivedEventArgs : EventArgs
        {
            ...
            public ReceivedEventArgs(Work WorkItem, byte Slave, byte Command, string Message, byte[] Raw)
            {
                this.Slave = Slave;
                this.Command = Command;
        ...
        protected override void OnParsePacket(byte[] baResponse, int nRecv, Work wi)
        {
            ...
            if (bValid && bComplete)
            {
                ...
                if (sum == ls[ls.Count - 1])
                {
                    byte slave = ls[0];
                    byte cmd = ls[1];
                    var ba = ls.GetRange(2, ls.Count - 3).ToArray();
                    var msg = MessageEncoding.GetString(ba);
                    MessageReceived(this, new ReceivedEventArgs(wi, slave, cmd, msg, ba));
                }
            }
        }
        #endregion
